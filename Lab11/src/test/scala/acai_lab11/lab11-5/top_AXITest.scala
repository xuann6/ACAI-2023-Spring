...
class top_AXITest(dut:top_AXI) extends PeekPokeTester(dut){
    ...
    var Mem_R_Stall_Count = 0
    var Mem_W_Stall_Count = 0
    var Mem_W_Count = 0
    var Mem_R_Count = 0
    ...
    while(!peek(dut.io.Hcf)){
        ...
        var MEM_STORE = peek(dut.io.MEM_STORE).toInt
        var MEM_LOAD = peek(dut.io.MEM_LOAD).toInt
        var LOAD_BYTES = peek(dut.io.MEM_L_Bytes).toInt
        var WRITE_BYTES = peek(dut.io.MEM_W_Bytes).toInt
       ...
       Cycle_Count += 1 //Cycle
        ...
        if(Stall_MA==0 && MEM_LOAD==1){
            Mem_R_Count += 1
            Mem_R_Bytes_Count += LOAD_BYTES
        }
        if(Stall_MA==0 && MEM_STORE==1){
            Mem_W_Count += 1
            Mem_W_Bytes_Count += WRITE_BYTES
        }
        /* Lab 10_3 performance counter */
    ...
    println(s"[Mem_R_Stall_Count              ] ${"%8d".format(Mem_R_Stall_Count)}")
    println(s"[Mem_W_Stall_Count              ] ${"%8d".format(Mem_W_Stall_Count)}")
    println(s"[Mem_R_Count                    ] ${"%8d".format(Mem_R_Count)}")
    println(s"[Mem_W_Count                    ] ${"%8d".format(Mem_W_Count)}")
    println(s"[Mem_R_Bytes_Count              ] ${"%8d".format(Mem_R_Bytes_Count)}")
    println(s"[Mem_W_Bytes_Count              ] ${"%8d".format(Mem_W_Bytes_Count)}")

    // Performance Analysis
    println("==============================================================")
    println("Performance Analysis:")
    println(s"[CPI                                       ] ${"%8f".format(Cycle_Count.toFloat/Inst_Count.toFloat)}")
    println(s"[Average Mem Read Request Stall Cycle      ] ${"%8f".format(Mem_R_Stall_Count.toFloat/Mem_R_Count.toFloat)}")
    println(s"[Average Mem Write Request Stall Cycle     ] ${"%8f".format(Mem_W_Stall_Count.toFloat/Mem_W_Count.toFloat)}")
    println(s"[Total Bus bandwidth requiement            ] ${"%8f".format(Mem_R_Bytes_Count.toFloat+Mem_W_Bytes_Count.toFloat)}")
    println("==============================================================")
    /* Lab 10_3 performance counter */

    /* Lab 11_5 performance counter */
    var cache_accesses = peek(dut.io.access_counts).toDouble
    var cache_hits = peek(dut.io.read_hits).toDouble + peek(dut.io.write_hits).toDouble
    var cache_misses = peek(dut.io.read_misses).toDouble + peek(dut.io.write_misses).toDouble

    var cache_hit_rate = cache_hits / cache_accesses
    println("==============================================================")
    println("Cache Performance Analysis:")
    println("cache access : " + cache_accesses.toString)
    println("cache hits : " + cache_hits.toString)
    println("cache misses : " + cache_misses.toString)
    println("cache hit rate : " + cache_hit_rate.toString)
    println("==============================================================")
