...
class Path_ID(addrWidth:Int) extends Module {
    val io = IO(new Bundle{
        ...
        val v_ID_rd_rdata = Output(UInt(512.W))
    })
    // Inst Decode
    val rs1_index = io.ID_inst_in(19,15)
    val rs2_index = io.ID_inst_in(24,20)
    val rd_index = io.ID_inst_in(11,7)
    ...
    val v_rf = Module(new Vector_RegFile(3))
    ...
    v_rf.io.vector_raddr(1) := rs2_index
    v_rf.io.vector_raddr(2) := rd_index
    io.v_ID_rs1_rdata:= v_rf.io.vector_rdata(0)
    io.v_ID_rs2_rdata:= v_rf.io.vector_rdata(1)
    io.v_ID_rd_rdata:= v_rf.io.vector_rdata(2)
