...
class Path_EXE(addrWidth:Int) extends Module {
    val io = IO(new Bundle{
        ...
        val v_EXE_rd_rdata_in = Input(UInt(512.W))
        val v_E_ASel = Input(Bool())
        val v_E_BSel = Input(Bool())
        val v_E_ALUSel = Input(UInt(4.W))
        val v_EXE_alu_out = Output(UInt(512.W))
        val v_EXE_rd_rdata_out = Output(UInt(512.W))
    })
    ...
    v_alu.io.vector_src1    := Mux(io.v_E_ASel === 0.U , io.v_EXE_rs1_rdata_in, Cat(0.U(480.W),io.EXE_rs1_rdata_in))
    v_alu.io.vector_src2    := Mux(io.v_E_BSel === 0.U , io.v_EXE_rs2_rdata_in, 0.U)
    v_alu.io.vector_ALUSel  := io.v_E_ALUSel
    io.v_EXE_alu_out := v_alu.io.vector_out
    io.v_EXE_rd_rdata_out := io.v_EXE_rd_rdata_in
}
