...
// import acai_lab11.Hw1._ // comment out this line since I only modified HW11-3
...
class Cache(val table_height: Int, val way: Int,val addrWidth: Int, val dataWidth: Int) extends Module {
  ...
  // (Hsuan) Copy from Lab 11-4
  val controller = Module(new controller_4_way)
  ...
  // (Hsuan) modified parameter
  io.axiIF.writeAddr.bits.addr := Cat(cacheMem.io.tagOut(controller.io.chosenWay)(18,0), io.addr.bits(12,5),0.U(5.W))
